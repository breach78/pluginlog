...
final class RetainedSetupFlowTests: XCTestCase {
  ...
  func testAppEntitlementsAllowVaultFolderSelectionAndPersistence() throws {
    let entitlementsURL = URL(fileURLWithPath: #filePath)
      ...
      .deletingLastPathComponent()
      .deletingLastPathComponent()
      .appendingPathComponent("import/BUF/BrainUnfog.entitlements", isDirectory: false)
    ...
    XCTAssertEqual(plist["com.apple.security.automation.apple-events"], true)
  ...
  func testConfigureObsidianVaultCreatesAppOwnedContainerWithoutRawProjectsAndRunsReminderFirstBootstrap()
    async throws
  {
  ...
  func testConfigureObsidianVaultLeavesExistingLegacyProjectNoteUnchangedWithoutImportingIt()
    async throws
  {
    let vaultRoot = try makeVaultRoot()
    let gateway = SetupReminderGateway()
    ...
    XCTAssertFalse(appProject.noteMarkdown.contains("Local prose that must stay."))
    XCTAssertEqual(appProject.title, "Imported list")
    XCTAssertEqual(appState.obsidianVaultRootURL?.standardizedFileURL, vaultRoot.standardizedFileURL)
    XCTAssertTrue(appState.hasCompletedInitialSetup)
...
private final class SetupReminderGateway: ReminderGateway {
  ...
  init(accessGranted: Bool = true, allowsProjectRemoval: Bool = false) {
    ...
    self.allowsProjectRemoval = allowsProjectRemoval
